...
export default function Home({ scrapeList }: { scrapeList: ScrapeItem[] }) {
  ...
  function markToCopy() {
    const copyText: HTMLTextAreaElement | null =
  ...
  return (
    <div className="container">
      ...
      <h1 className="text-center">Web Scraping Demonstrations</h1>
      <h5 className="text-right">
        React Native Expo Publish:{" "}
        <a
          href="https://exp.host/@jglchen/web-scrape"
          target="_blank"
          rel="noreferrer"
        >
          https://exp.host/@jglchen/web-scrape
        </a>
      </h5>
      <form>
      ...
      <div className="clearfix">
        <h4 className="space-between">
          ...
          <button className="accent-button button-right" onClick={goWebScrape}>
            Go
          </button>
        </h4>
      </div>
      <div
        className="clearfix"
        dangerouslySetInnerHTML={{ __html: getPostsData(url, scrapeList) }}
      />
      <div className="space-between2">
        <div>
          <input
            ...
            onChange={toJSON}
          />
          <label htmlFor="JSON" className={labelStyles.horizontal}>
            JSON
          </label>
          <input
            type="radio"
            ...
            onChange={toCSV}
          />
          <label htmlFor="CSV" className={labelStyles.horizontal}>
            CSV
          </label>
          <input
            type="radio"
            ...
            name="strFormat"
            value="XML"
            checked={dataType === "XML"}
            onChange={toXML}
          />
          <label htmlFor="XML" className={labelStyles.horizontal}>
            XML
          </label>
          <input
            type="radio"
            ...
            name="strFormat"
            value="YAML"
            checked={dataType === "YAML"}
            onChange={toYAML}
          />
          <label htmlFor="YAML" className={labelStyles.horizontal}>
            YAML
          </label>
          <input
            type="radio"
            ...
            name="strFormat"
            value="MySQL"
            checked={dataType === "MySQL"}
            onChange={toMySQL}
          />
          <label htmlFor="MySQL" className={labelStyles.horizontal}>
            MySQL
          </label>
        </div>
        {extractData && !extractData.startsWith("Error") && (
          <div>
            <button className="muted-button" onClick={handleMailSend}>
              {goMail ? "Send Mail" : "Mail Result"}
            </button>
            <button className="muted-button button-right" onClick={markToCopy}>
              Copy to Clipboard
            </button>
          </div>
        )}
      ...
      {inScrape && (
        <div className={loaderStyles.loadermodal}>
          <div
            className={`${loaderStyles.loader} ${loaderStyles.div_on_center}`}
          />
        </div>
      )}
    </div>
